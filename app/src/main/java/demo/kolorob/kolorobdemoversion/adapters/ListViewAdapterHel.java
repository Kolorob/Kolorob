...
public class ListViewAdapterHel extends BaseAdapter {

	// Declare Variables
	Context mContext;
	LayoutInflater inflater;
	Activity activity;
	private List<PopulatedfromDBHel> worldpopulationlist = null;
	private ArrayList<PopulatedfromDBHel> arraylist;
HealthServiceProviderItem nullfin;
	public ListViewAdapterHel(Activity act, List<PopulatedfromDBHel> worldpopulationlist) {
		this.activity = act;
		this.worldpopulationlist = worldpopulationlist;

		inflater = act.getLayoutInflater();
		this.arraylist = new ArrayList<PopulatedfromDBHel>();
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		view.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				HealthServiceProviderTable healthServiceProviderTable=new HealthServiceProviderTable(ListViewAdapterHel.this.mContext);
				nullfin=healthServiceProviderTable.gethelNode2(worldpopulationlist.get(position).getNodeid());
				Intent iient = new Intent(getActivity(), DetailsInfoActivityHealth.class);
				iient.putExtra(AppConstants.KEY_DETAILS_HEALTH, nullfin);
...
//				Toast.makeText(mContext, "rank " + worldpopulationlist.get(position).getNodeid(), Toast.LENGTH_LONG).show();
	...
	public void filter(String charText) {
		...
		{
			for (PopulatedfromDBHel wp : arraylist)
			{
